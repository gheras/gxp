...
gxp.slider.TimeSlider = Ext.extend(Ext.slider.MultiSlider, {
    ...
    onModelModified : function(evt){
        ...
        var lastInfo = lastLayer.metadata.timeInfo;
        ...
        new Ext.util.DelayedTask(this.manageConflict, this, [this.conflicts]).delay(100);
    },
    ...
    manageConflict: function(conflicts){
        var lastLayer = this.model.layers.slice(-1)[0];
        Ext.Msg.confirm('Time Conflicts Found', 'There were '+ conflicts.length + ' inconsistencies between the layer you just added and the ' +
            ...
            function(btn, text){
                if(btn == 'yes'){
                    ...
                    lastLayer.map.removeLayer(lastLayer);
                }
            },
            this);
    }
